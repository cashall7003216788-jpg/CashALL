...
import { Printer, ShieldCheck, Download, CheckCircle2, FileText, ArrowLeft, Loader2 } from "lucide-react";
...
export default function CustomerBillPage() {
  ...
  useEffect(() => {
    // Instant cache pre-population if available locally
      ...
      if (stored) {
        try {
          ...
          if (parsed.orderNumber === orderId || parsed.id === orderId) {
            setBillData((prev: any) => prev || {
              billNumber: `${parsed.orderNumber || orderId}-${new Date().getFullYear()}`,
              orderNumber: parsed.orderNumber || orderId,
              seller: {
                name: parsed.customerName || "Customer",
                phoneMasked: parsed.customerPhone ? `+91 ${parsed.customerPhone.slice(0, 2)}****${parsed.customerPhone.slice(-4)}` : "—",
                address: parsed.addressSummary || "Howrah, West Bengal",
              },
              device: {
                deviceName: cleanDeviceName(parsed.deviceName || "Mobile Device"),
                variant: "128 GB",
                imei1: "—",
              },
              financials: {
                finalPurchasePrice: parsed.revisedPrice || parsed.estimatedPrice || 0,
                paymentMethod: "Instant UPI / Bank Transfer",
                utrNumber: "—",
              },
              declarations: {
                eSignTimestamp: parsed.createdAt || new Date().toISOString(),
              },
            });
          }
        } catch {}
    ...
    const fetchBill = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.data?.billData) {
            setBillData(data.data.billData);
          }
        }
      } catch (err) {
    ...
    <div className="min-h-screen flex flex-col bg-brand-bg text-brand-black">
      ...
      <main className="flex-grow py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {loading && !billData ? (
            <div className="bg-white rounded-3xl p-12 text-center border border-brand-border shadow-premium space-y-4">
              <Loader2 className="w-8 h-8 animate-spin text-brand-yellow mx-auto" />
              <p className="text-sm font-bold text-brand-black">Loading Official Bill &amp; Tax Invoice...</p>
              <p className="text-xs text-brand-muted">Fetching verified sale agreement, IMEI records &amp; payment reference.</p>
            </div>
          ) : (
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-1">
                    ...
                    <div className="font-extrabold text-brand-black text-sm">{cleanDeviceName(billData?.device?.deviceName || `${billData?.device?.brand || ""} ${billData?.device?.model || ""}`.trim() || "Mobile Device")}</div>
