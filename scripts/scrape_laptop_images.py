...
with open('dataset used/final_laptops.txt', 'r', encoding='utf-8') as f:
    text = f.read()

blocks = re.findall(r'MODEL\s*:\s*([^\n]+)\s*\n\s*Cashify URL\s*:\s*(https?://[^\s]+)', text)
print(f"Found {len(blocks)} laptop model+URL pairs in final_laptops.txt")
...
print("Fetching laptop product images from Cashify...")
...
print(f"Successfully scraped {len(results)} laptop image mappings!")
with open('dataset used/cashify_laptop_images.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, indent=2)
print("Saved dataset used/cashify_laptop_images.json")
