"""
Master Script: Cashify Mobile Phones Dataset Generator v3
1. Extracts base models per brand from Cashify.
2. Follows Cashify's exact "Choose a variant" options (e.g. iPhone 6 Plus -> 16GB, 64GB, 128GB).
3. Fetches exact "Get Upto ₹XX,XXX" Cashify resale valuation price for EVERY variant.
4. NO RELEASE YEAR (per user instruction: "LEAVE THE RELEASE YEAR DONT NEED TO MENTION RELEASE YEAR").
5. Produces Cashify_Mobile_Phones_Dataset.txt for user review.
"""
...
# Master dataset dictionary: brand_name -> list of model objects
DATASET_MAP = {}
ALL_VARIANT_TASKS = []


def fetch_variant_price_and_image(var_item):
    url = var_item['url']
...
async def main():
    async with async_playwright() as p:
        ...
        print(f"Total Unique Mobile Brands Discovered: {len(unique_brands)}")

        base_model_urls = []

        print("\n=== STEP 2: Collecting Base Model Pages per Brand ===")
        for b_idx, brand in enumerate(unique_brands, 1):
            ...
            try:
                ...
                for m in models_data:
                    m_url = m['href']
                    # Keep base model pages (pages without explicit variant -gb/ram patterns or primary model entry)
        ...
        print(f"\n=== STEP 3: Discovering Variants for {len(base_model_urls)} Base Models ===")
        
        # Concurrency semaphore for model pages
            ...
            async with sem:
                ...
                try:
                    ...
                    if variant_options:
                        ...
                        # Fallback single variant if page has no additional variant chips
                        ram = "Standard"
                        storage = "Standard"
        ...
        v_tasks = [inspect_model_variants(mi) for mi in base_model_urls]
        await asyncio.gather(*v_tasks)
        await browser.close()

    print(f"\n=== STEP 4: Multithreaded Price Fetching ({len(ALL_VARIANT_TASKS)} Variants) ===")
    with concurrent.futures.ThreadPoolExecutor(max_workers=40) as executor:
        executor.map(fetch_variant_price_and_image, ALL_VARIANT_TASKS)
